...
# =============================================================================
# LISTINGADAPTER â€" Bridge parser dicts to classifier objects
# =============================================================================

class ListingAdapter:
    """
    Adapter: dict â†' object with attribute access.
...
def apply_filters(listings: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Filters already applied by parser:
    - time_ok (max_time_hours)
    - qty_ok (default_min_qty)
    - blacklist_ok (keyword_blacklist)
    
    Returns only listings where all filter_flags are True.
    """
    eligible = []
    
    for listing in listings:
        flags = listing.get("filter_flags", {})
...
def capture_ema_updates(eligible: List[Dict[str, Any]], store: dict) -> int:
    ...
    Contract: v1.2 Section 7
    """
    updates = []
    
    for listing in eligible:
        # EMA capture eligibility (per Contract v1.2 Section 7)
        bids = listing.get("bids", 0)
        
        # Skip if no bids and PRICE_CAPTURE_ONLY_IF_BIDS is True
        if config.PRICE_CAPTURE_ONLY_IF_BIDS and bids == 0:
            continue
    ...
    # Capture updates
    if updates:
        price_store.capture_ema_from_listings(updates)
...
def run_once() -> dict:
    ...
    store = price_store.load_price_store()
    captured = capture_ema_updates(eligible, store)
    if captured > 0:
        price_store.save_price_store(store)
...
def main():
    ...
    while True:
        ...
        if email_sent:
            ...
            subject = f"{earliest_time} Offline eBay Silver HITS ({result['new_hit_count']} new)"
